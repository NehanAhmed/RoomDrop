...
```
Uses `pnpm` (lockfile: `pnpm-lock.yaml`). No typecheck or db:push script exists.
...
- **Env validation**: `lib/env.ts` throws at startup if any required var is missing.
...
- **CI/CD**: GitHub Actions runs lint → build → test on push/PR. Husky pre-commit hook runs lint-staged.
